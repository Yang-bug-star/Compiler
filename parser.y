...
%token <node> CHAR INT DOUBLE VOID SIZEOF
%token <node> IF ELSE SWITCH CASE DEFAULT WHILE DO FOR GOTO CONTINUE BREAK RETURN
%token <node> IDENTIFIER INT_CONSTANT
%token <node> ';' ',' ':' '=' '[' ']' '{' '}' '(' ')' '.' '&' '|' '^' '!' '~' '+' '-' '*' '/' '%' '<' '>'  '?' 
%token <node> LEFT_SHIFT_OP RIGHT_SHIFT_OP LE_OP GE_OP EQ_OP NE_OP PTR_OP INC_OP DEC_OP 
%token <node> ADD_ASSIGN SUB_ASSIGN MUL_ASSIGN DIV_ASSIGN MOD_ASSIGN LEFT_SHIFT_ASSIGN RIGHT_SHIFT_ASSIGN AND_ASSIGN XOR_ASSIGN OR_ASSIGN
...
%type <node> program compound_statement 
...
declarator:
    ...
    | declarator '(' identifier_list ')'{//函数调用
      $$=new TreeNode("declarator",4,$1,$2,$3,$4);      
    }
    | declarator '(' ')'{//无参函数
      $$=new TreeNode("declarator",3,$1,$2,$3);    
...
selection_statement:
    ...
    | IF '(' expression ')' statement %prec LOWER_PREC_THAN_ELSE{//这条语句的优先级更低
...
logical_and_expression:
      inclusive_or_expression{//按位或表达式 |
...
inclusive_or_expression:
      exclusive_or_expression{//按位异或表达式 ^
